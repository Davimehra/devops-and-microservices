...
const VerifyAuthPage_SignUp_User = () => {
    ...
    const restauratIdInputRef = useRef();
    ...
    const error_RestaurantId_Ref = useRef();
    ...
    const onRestaruantIdInput = (input) => {
        setRestaurantId(input?.target?.value)
        if (input?.target?.value === undefined || input?.target?.value === null || String(input?.target?.value).length <= 0) {
            setEmptyRestaurantId(true)
        } else {
            setEmptyRestaurantId(false)
        }
    }
    ...
    const onClickSwitch = () => {
        const leftCal = document?.querySelector("#switchId")?.style?.getPropertyValue('left');
        if (leftCal <= '30%') {
            document?.querySelector("#switchId")?.style?.setProperty('left', '40%');
            setUserType("employee")
        } else {
            document?.querySelector("#switchId")?.style?.setProperty('left', '10%')
    ...
    const onClickSubmit = async () => {
        ...
        restauratIdInputRef?.current?.style?.setProperty("--input-border-color", "grey")



        if (userType === 'employee') {
            allVerifyFeild.push(verifyErrors({ feildValue: restauratIdInputRef?.current?.value, feildType: 'restaurantId', error_feild_Ref: error_RestaurantId_Ref }))
        }
        ...
        verifyResult?.forEach(async (result, index) => {
            if (result?.status == 'rejected') {
                if (result?.reason instanceof NameVerificationError) {
                    ...
                    nameInputRef?.current?.style?.setProperty("--input-border-color", "red")
                    errorCount += 1;
                }
            }
            if (result?.status == 'rejected') {
            ...
            if (result?.status == 'rejected') {
                if (result?.reason instanceof RestaurantIdVerificationError) {
                    error_RestaurantId_Ref.current.innerHTML = `<div style="color:red">${languagesTitle?.Restaurant[currentLanguage]} ${languagesTitle?.Id[currentLanguage]}</div> <div> Length 1-10, only digits </div>`
                    restauratIdInputRef?.current?.style?.setProperty("--input-border-color", "red")
                    errorCount += 1;
                }
            }
            if (result?.status == 'rejected') {
            ...
            if (index == verifyResult?.length - 1) {
                ...
                if (errorCount >= 1) {
                    ...
                    await axiosConfig({
                        ...
                        params: { secret: sessionSecret, verifyOtp: false, userType },
                        method: 'post',
                        data: userType === 'customer' ?
                            { email: email, password: password, confirmPassword: password, name: name } :
                            { email: email, password: password, confirmPassword: password, restaurantId: restauratIdInputRef?.current?.value, name: name, userType },
                        contentType: "application/json",
                    }).then((res) => {
                        if (userType === 'customer') {
                            navigate(`/va/signup/verifyem?email=${btoa(email)}&sessionsecret=${btoa(sessionSecret)}&verifyOTP=false&pass=${btoa(password)}&name=${btoa(name)}&signuptype=${btoa(signupType)}&usertype=${btoa("customer")}&restaurantid=`, { replace: true })
                        } else {
                            navigate(`/va/signup/verifyem?email=${btoa(email)}&sessionsecret=${btoa(sessionSecret)}&verifyOTP=false&pass=${btoa(password)}&name=${btoa(name)}&restaurantid=${btoa(restaurantId)}&signuptype=${btoa(signupType)}&usertype=${btoa("employee")}`, { replace: true })
                        }
                    }).catch((error) => {
                    ...
                    <div className={VerifyAuthPageCss.ContentContainer}>
                        ...
                        <div className={VerifyAuthPageCss.ContentBlock}>
                            ...
                            <div ref={authContentRef} className={VerifyAuthPageCss.Content}>
                                <div className={classNames(VerifyAuthPageCss.textSizeLarge, VerifyAuthPageCss.textBold)}>{languagesTitle?.CreateAccount[currentLanguage]}</div>
                                ...
                                <div flexdirection='row' className={VerifyAuthPageCss.InputBlock}>
                                    ...
                                    <div tabIndex={1} onFocus={onFocusTab} onClick={onClickSwitchEmail} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickSwitch() } }} className={classNames(VerifyAuthPageCss.textSizeSmall, VerifyAuthPageCss?.Link, VerifyAuthPageCss?.CursorPointer)}>{languagesTitle?.Switch[currentLanguage]}</div>
                                </div>

                                {/* InputBlock  - Switch*/}
                                <div flexdirection='row' className={VerifyAuthPageCss.InputBlock}>
                                    <div tabIndex={2} onFocus={onFocusTab} onClick={onClickSwitch} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickSwitch() } }} className={classNames(VerifyAuthPageCss?.SwitchBox, VerifyAuthPageCss?.CursorPointer)}>
                                        <div id="switchId" className={VerifyAuthPageCss?.Switch}></div>
                                    </div>
                                    <div className={classNames(VerifyAuthPageCss?.SwitchBoxStatus, VerifyAuthPageCss?.textSizeSmall)}>{userType === 'customer' ? 'Sign up as Employee' : "Sign up as Customer"}</div>
                                </div>
                                ...
                                {userType === 'employee' ?
                                    <div className={VerifyAuthPageCss.InputBlock}>
                                        <div className={VerifyAuthPageCss.InputHeadingBlock}>
                                            <label className={classNames(VerifyAuthPageCss.textSizeMedium, VerifyAuthPageCss.textBold)}>{`${languagesTitle?.Restaurant[currentLanguage]} ${languagesTitle?.Id[currentLanguage]}`}</label>
                                            {emptyRestaurantId == false ? <div tabIndex={3} className={classNames(VerifyAuthPageCss?.CursorPointer, VerifyAuthPageCss?.textSizeSmall, VerifyAuthPageCss?.Link)} onFocus={onFocusTab} onClick={() => { setRestaurantId(''); setEmptyRestaurantId(true) }} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { setRestaurantId(''); setEmptyRestaurantId(true) } }}>{`${languagesTitle?.Clear[currentLanguage]}`} </div> : <div></div>}
                                        </div>
                                        <input onFocus={onFocusTab} tabIndex={4} placeholder={languagesTitle?.EnterRestaurantId[currentLanguage]} className={classNames(VerifyAuthPageCss?.InputBlock_Input, VerifyAuthPageCss?.Animation_Shake1deg)} type="text" ref={restauratIdInputRef} onInput={onRestaruantIdInput} value={restaurantId}></input>
                                        <div ref={error_RestaurantId_Ref} className={classNames(VerifyAuthPageCss.ErrorFeild, VerifyAuthPageCss?.textSizeSmall)}></div>
                                    </div> : null
                                }
                                {/* minHeading Name*/}
                                ...
                                <div className={VerifyAuthPageCss.InputBlock}>
                                    <div className={VerifyAuthPageCss.InputHeadingBlock}>
                                        ...
                                        {emptyName == false ? <div tabIndex={5} className={classNames(VerifyAuthPageCss?.CursorPointer, VerifyAuthPageCss?.textSizeSmall, VerifyAuthPageCss?.Link)} onFocus={onFocusTab} onClick={() => { setName(''); setEmptyName(true) }} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { setName(''); setEmptyName(true) } }}>{`${languagesTitle?.Clear[currentLanguage]}`}</div> : <div></div>}
                                    </div>
                                    <input onFocus={onFocusTab} tabIndex={6} onChange={(event) => { setName(event?.target?.value) }} placeholder={languagesTitle?.EnterFirstAndLastName[currentLanguage]} className={classNames(VerifyAuthPageCss?.InputBlock_Input, VerifyAuthPageCss?.Animation_Shake1deg)} type="text" ref={nameInputRef} onInput={onNameInput} value={name} ></input>
                                ...
                                <div className={VerifyAuthPageCss.InputBlock}>
                                    <div className={VerifyAuthPageCss.InputHeadingBlock}>
                                        ...
                                        {emptyPassword == false ? <div tabIndex={7} className={classNames(VerifyAuthPageCss?.CursorPointer, VerifyAuthPageCss?.textSizeSmall, VerifyAuthPageCss?.Link)} onFocus={onFocusTab} onClick={() => { setPassword(''); setEmptyPassword(true) }} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { setPassword(''); setEmptyPassword(true) } }}>{`${languagesTitle?.Clear[currentLanguage]}`}</div> : <div></div>}
                                    </div>
                                    <input onFocus={onFocusTab} tabIndex={8} onChange={(event) => { setPassword(event?.target?.value) }} className={classNames(VerifyAuthPageCss?.InputBlock_Input, VerifyAuthPageCss?.Animation_Shake1deg)} maxLength={16} type="text" ref={passwordInputRef} onInput={onPasswordInput} value={password} ></input>
                                ...
                                <div tabIndex={9} onFocus={onFocusTab} onClick={onClickSubmit} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickSubmit() } }} ref={continueButtonRef} className={classNames(VerifyAuthPageCss?.ContinueButton, VerifyAuthPageCss?.Animation_Shake1deg)}>
                                    {isLoading ?
                    ...
                    <div className={VerifyAuthPage_Portait_Css.ContentContainer}>
                        ...
                        <div className={VerifyAuthPage_Portait_Css.ContentBlock}>
                            ...
                            <div ref={authContentRef} className={VerifyAuthPage_Portait_Css.Content}>
                                <div className={classNames(VerifyAuthPage_Portait_Css.textSizeLarge, VerifyAuthPage_Portait_Css.textBold)}>{languagesTitle?.CreateAccount[currentLanguage]}</div>
                                ...
                                <div flexdirection='row' className={VerifyAuthPage_Portait_Css.InputBlock}>
                                    ...
                                    <div tabIndex={1} onFocus={onFocusTab} onClick={onClickSwitchEmail} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickSwitch() } }} className={classNames(VerifyAuthPage_Portait_Css.textSizeSmall, VerifyAuthPage_Portait_Css?.Link, VerifyAuthPage_Portait_Css?.CursorPointer)}>{languagesTitle?.Switch[currentLanguage]}</div>
                                </div>

                                {/* InputBlock  - Switch*/}
                                <div flexdirection='row' className={VerifyAuthPage_Portait_Css.InputBlock}>
                                    <div tabIndex={2} onFocus={onFocusTab} onClick={onClickSwitch} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickSwitch() } }} className={classNames(VerifyAuthPage_Portait_Css?.SwitchBox, VerifyAuthPage_Portait_Css?.CursorPointer)}>
                                        <div id="switchId" className={VerifyAuthPage_Portait_Css?.Switch}></div>
                                    </div>
                                    <div className={classNames(VerifyAuthPage_Portait_Css?.SwitchBoxStatus, VerifyAuthPage_Portait_Css?.textSizeSmall)}>{userType === 'customer' ? 'Sign up as Employee' : "Sign up as Customer"}</div>
                                </div>
                                ...
                                {userType === 'employee' ?
                                    <div className={VerifyAuthPage_Portait_Css.InputBlock}>
                                        <div className={VerifyAuthPage_Portait_Css.InputHeadingBlock}>
                                            <label className={classNames(VerifyAuthPage_Portait_Css.textSizeMedium, VerifyAuthPage_Portait_Css.textBold)}>{`${languagesTitle?.Restaurant[currentLanguage]} ${languagesTitle?.Id[currentLanguage]}`}</label>
                                            {emptyRestaurantId == false ? <div tabIndex={3} className={classNames(VerifyAuthPage_Portait_Css?.CursorPointer, VerifyAuthPage_Portait_Css?.textSizeSmall, VerifyAuthPage_Portait_Css?.Link)} onFocus={onFocusTab} onClick={() => { setRestaurantId(''); setEmptyRestaurantId(true) }} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { setRestaurantId(''); setEmptyRestaurantId(true) } }}>{`${languagesTitle?.Clear[currentLanguage]}`} </div> : <div></div>}
                                        </div>
                                        <input onFocus={onFocusTab} tabIndex={4} placeholder={languagesTitle?.EnterRestaurantId[currentLanguage]} className={classNames(VerifyAuthPage_Portait_Css?.InputBlock_Input, VerifyAuthPage_Portait_Css?.Animation_Shake1deg)} type="text" ref={restauratIdInputRef} onInput={onRestaruantIdInput} value={restaurantId}></input>
                                        <div ref={error_RestaurantId_Ref} className={classNames(VerifyAuthPage_Portait_Css.ErrorFeild, VerifyAuthPage_Portait_Css?.textSizeSmall)}></div>
                                    </div> : null
                                }
                                {/* minHeading Name*/}
                                ...
                                <div className={VerifyAuthPage_Portait_Css.InputBlock}>
                                    <div className={VerifyAuthPage_Portait_Css.InputHeadingBlock}>
                                        ...
                                        {emptyName == false ? <div tabIndex={5} className={classNames(VerifyAuthPage_Portait_Css?.CursorPointer, VerifyAuthPage_Portait_Css?.textSizeSmall, VerifyAuthPage_Portait_Css?.Link)} onFocus={onFocusTab} onClick={() => { setName(''); setEmptyName(true) }} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { setName(''); setEmptyName(true) } }}>{`${languagesTitle?.Clear[currentLanguage]}`}</div> : <div></div>}
                                    </div>
                                    <input onFocus={onFocusTab} tabIndex={6} onChange={(event) => { setName(event?.target?.value) }} placeholder={languagesTitle?.EnterFirstAndLastName[currentLanguage]} className={classNames(VerifyAuthPage_Portait_Css?.InputBlock_Input, VerifyAuthPage_Portait_Css?.Animation_Shake1deg)} type="text" ref={nameInputRef} onInput={onNameInput} value={name} ></input>
                                ...
                                <div className={VerifyAuthPage_Portait_Css.InputBlock}>
                                    <div className={VerifyAuthPage_Portait_Css.InputHeadingBlock}>
                                        ...
                                        {emptyPassword == false ? <div tabIndex={7} className={classNames(VerifyAuthPage_Portait_Css?.CursorPointer, VerifyAuthPage_Portait_Css?.textSizeSmall, VerifyAuthPage_Portait_Css?.Link)} onFocus={onFocusTab} onClick={() => { setPassword(''); setEmptyPassword(true) }} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { setPassword(''); setEmptyPassword(true) } }}>{`${languagesTitle?.Clear[currentLanguage]}`}</div> : <div></div>}
                                    </div>
                                    <input onFocus={onFocusTab} tabIndex={8} onChange={(event) => { setPassword(event?.target?.value) }} className={classNames(VerifyAuthPage_Portait_Css?.InputBlock_Input, VerifyAuthPage_Portait_Css?.Animation_Shake1deg)} maxLength={16} type="text" ref={passwordInputRef} onInput={onPasswordInput} value={password} ></input>
                                ...
                                <div tabIndex={9} onFocus={onFocusTab} onClick={onClickSubmit} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickSubmit() } }} ref={continueButtonRef} className={classNames(VerifyAuthPage_Portait_Css?.ContinueButton, VerifyAuthPage_Portait_Css?.Animation_Shake1deg)}>
                                    {isLoading ?
                                        <img src={loadingImage} width={'30px'} height={'30px'}></img> :
...
export default VerifyAuthPage_SignUp_User;