...
const VerifyAuthPage_UpdatePassword = () => {
    const [emptyPassword, setEmptyPassword] = useState(true);
    const [emptyConfirmPassword, setEmptyConfirmPassword] = useState(true);
    ...
    const error_Confirm_Password_Ref = useRef();
    ...
    useEffect(() => {
        ...
        setPasswordSecret(searchParams?.get('passwordsecret'));
        if (!searchParams.get('email')) {
            navigate('/va', { replace: true })
        }
        if (!searchParams?.get('passwordsecret')) {
            navigate('/va', { replace: true });
        }
    }, [])
    ...
    const onConfirmPasswordInput = (input) => {
        if (input?.target?.value === undefined || input?.target?.value === null || String(input?.target?.value).length == 0) {
    ...
    const onClickClear_ConfirmPassword = () => {
    ...
    const onClickShow_ConfirmPassword = () => {
        if (String(confirmPasswordInputRef.current.type) === 'password' | String(confirmPasswordInputRef.current.type) === null) {
            ...
            if (index == verifyResult?.length - 1) {
                ...
                if (errorCount >= 1) {
                    ...
                    if (passwordInputRef?.current?.value != confirmPasswordInputRef?.current?.value) {
                        error_Confirm_Password_Ref.current.innerHTML = `<div style="color:red">${languagesTitle?.ConfirmPassword[currentLanguage]}</div> <div> Password does'nt match</div>`;
                        error_Confirm_Password_Ref?.current?.style?.setProperty('visibility', 'visible');
                        error_Confirm_Password_Ref?.current?.style?.setProperty('display', 'flex');
                    } else {
                        error_Confirm_Password_Ref?.current?.style?.setProperty('visibility', 'hidden');
                        error_Confirm_Password_Ref?.current?.style?.setProperty('display', 'none');
                    }
                    // Success
                    // Do Submit Code Here ***
                    ...
                    await axiosConfig({
                        ...
                        params: { getLink: false },
                        method: 'post',
                        data: { email: email, newPassword: passwordInputRef?.current?.value, passwordSecret },
                        contentType: "application/json",
                    }).then((res) => {
                        if (res?.data?.message) {
                            window?.alert(res?.data?.message)
                        }
                        navigate(`/va/signin?email=${btoa(email)}`, { replace: true })
                    }).catch((error) => {
                    ...
                    <div className={VerifyAuthPageCss.ContentContainer}>
                        <div className={VerifyAuthPageCss.ContentBlock}>
                            ...
                            <div ref={authContentRef} className={VerifyAuthPageCss.Content}>
                                {/* MainHeading */}
                                <div className={classNames(VerifyAuthPageCss.textSizeLarge, VerifyAuthPageCss.textBold)}>{languagesTitle?.ChangePassword[currentLanguage]}</div>
                                ...
                                <div className={VerifyAuthPageCss.InputBlock}>
                                    <div className={VerifyAuthPageCss.InputHeadingBlock}>
                                        <label className={classNames(VerifyAuthPageCss.textSizeMedium, VerifyAuthPageCss.textBold)}>{languagesTitle?.NewPassword[currentLanguage]}</label>
                                        {emptyPassword == false ? <div className={classNames(VerifyAuthPageCss?.CursorPointer, VerifyAuthPageCss?.textSizeSmall, VerifyAuthPageCss?.Link)} onFocus={onFocusTab} onClick={onClickClear} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickClear() } }}>{`${languagesTitle?.Clear[currentLanguage]}`}</div> : <div></div>}
                                    </div>
                                    ...
                                    <div flexdirection='row' flexjustify='flex-end' >
                                        {emptyPassword == false ?
                                ...
                                {/* InputBlock1 - ConfirmPassword */}

                                <div className={VerifyAuthPageCss.InputBlock}>
                                    <div className={VerifyAuthPageCss.InputHeadingBlock}>
                                        <label className={classNames(VerifyAuthPageCss.textSizeMedium, VerifyAuthPageCss.textBold)}>{languagesTitle?.ConfirmPassword[currentLanguage]}</label>
                                        {emptyConfirmPassword == false ? <div className={classNames(VerifyAuthPageCss?.CursorPointer, VerifyAuthPageCss?.textSizeSmall, VerifyAuthPageCss?.Link)} onFocus={onFocusTab} onClick={onClickClear_ConfirmPassword} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickClear_ConfirmPassword() } }}>{`${languagesTitle?.Clear[currentLanguage]}`}</div> : <div></div>}
                                    </div>
                                    <input onFocus={onFocusTab} tabIndex={3} className={classNames(VerifyAuthPageCss?.InputBlock_Input, VerifyAuthPageCss?.Animation_Shake1deg)} maxLength={16} type="password" ref={confirmPasswordInputRef} onInput={onConfirmPasswordInput} ></input>
                                    <div flexdirection='row' flexjustify='flex-end' >
                                        {emptyConfirmPassword == false ?
                                            (isShowingConfirmPassword == false ?
                                                <img src={passwordShowIcon} width={'30px'} height={'30px'} tabIndex={2} onFocus={onFocusTab} onClick={onClickShow_ConfirmPassword} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickShow_ConfirmPassword() } }} className={classNames(VerifyAuthPageCss?.CursorPointer, VerifyAuthPageCss?.textSizeSmall, VerifyAuthPageCss?.Link)}>
                                                </img> :
                                                <img src={passwordHideIcon} width={`${0.055 * 25}vw`} height={`${0.055 * 25}vw`} tabIndex={2} onFocus={onFocusTab} onClick={onClickShow_ConfirmPassword} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickShow_ConfirmPassword() } }} className={classNames(VerifyAuthPageCss?.CursorPointer, VerifyAuthPageCss?.textSizeSmall, VerifyAuthPageCss?.Link)}>
                                                </img>
                                            )
                                    ...
                                    <div ref={error_Confirm_Password_Ref} className={classNames(VerifyAuthPageCss.ErrorFeild, VerifyAuthPageCss?.textSizeSmall)}></div>
                                </div>
                    ...
                    <div className={VerifyAuthPage_Portait_Css.ContentContainer}>
                        <div className={VerifyAuthPage_Portait_Css.ContentBlock}>
                            ...
                            <div ref={authContentRef} className={VerifyAuthPage_Portait_Css.Content}>
                                {/* MainHeading */}
                                <div className={classNames(VerifyAuthPage_Portait_Css.textSizeLarge, VerifyAuthPage_Portait_Css.textBold)}>{languagesTitle?.ChangePassword[currentLanguage]}</div>
                                ...
                                <div className={VerifyAuthPage_Portait_Css.InputBlock}>
                                    <div className={VerifyAuthPage_Portait_Css.InputHeadingBlock}>
                                        <label className={classNames(VerifyAuthPage_Portait_Css.textSizeMedium, VerifyAuthPage_Portait_Css.textBold)}>{languagesTitle?.NewPassword[currentLanguage]}</label>
                                        {emptyPassword == false ? <div className={classNames(VerifyAuthPage_Portait_Css?.CursorPointer, VerifyAuthPage_Portait_Css?.textSizeSmall, VerifyAuthPage_Portait_Css?.Link)} onFocus={onFocusTab} onClick={onClickClear} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickClear() } }}>{`${languagesTitle?.Clear[currentLanguage]}`}</div> : <div></div>}
                                    </div>
                                ...
                                <div className={VerifyAuthPage_Portait_Css.InputBlock}>
                                    <div className={VerifyAuthPage_Portait_Css.InputHeadingBlock}>
                                        <label className={classNames(VerifyAuthPage_Portait_Css.textSizeMedium, VerifyAuthPage_Portait_Css.textBold)}>{languagesTitle?.ConfirmPassword[currentLanguage]}</label>
                                        {emptyConfirmPassword == false ? <div className={classNames(VerifyAuthPage_Portait_Css?.CursorPointer, VerifyAuthPage_Portait_Css?.textSizeSmall, VerifyAuthPage_Portait_Css?.Link)} onFocus={onFocusTab} onClick={onClickClear_ConfirmPassword} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickClear_ConfirmPassword() } }}>{`${languagesTitle?.Clear[currentLanguage]}`}</div> : <div></div>}
                                    </div>
                                    <input onFocus={onFocusTab} tabIndex={3} className={classNames(VerifyAuthPage_Portait_Css?.InputBlock_Input, VerifyAuthPage_Portait_Css?.Animation_Shake1deg)} maxLength={16} type="password" ref={confirmPasswordInputRef} onInput={onConfirmPasswordInput} ></input>
                                    <div flexdirection='row' flexjustify='flex-end' >
                                        {emptyConfirmPassword == false ?
                                            (isShowingConfirmPassword == false ?
                                                <img src={passwordShowIcon} width={'30px'} height={'30px'} tabIndex={2} onFocus={onFocusTab} onClick={onClickShow_ConfirmPassword} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickShow_ConfirmPassword() } }} className={classNames(VerifyAuthPage_Portait_Css?.CursorPointer, VerifyAuthPage_Portait_Css?.textSizeSmall, VerifyAuthPage_Portait_Css?.Link)}>
                                                </img> :
                                                <img src={passwordHideIcon} width={`${0.055 * 25}vw`} height={`${0.055 * 25}vw`} tabIndex={2} onFocus={onFocusTab} onClick={onClickShow_ConfirmPassword} onKeyDown={(event) => { if (event?.code == 'Space' || event?.code == 'Enter') { onClickShow_ConfirmPassword() } }} className={classNames(VerifyAuthPage_Portait_Css?.CursorPointer, VerifyAuthPage_Portait_Css?.textSizeSmall, VerifyAuthPage_Portait_Css?.Link)}>
                                                </img>
                                            )
                                    ...
                                    <div ref={error_Confirm_Password_Ref} className={classNames(VerifyAuthPage_Portait_Css.ErrorFeild, VerifyAuthPage_Portait_Css?.textSizeSmall)}></div>
                                </div>
...
export default VerifyAuthPage_UpdatePassword;